...
struct clast_term *
new_clast_term (Value c, const char *v)
{
...
struct clast_binary *
new_clast_binary (enum clast_bin_type t, struct clast_expr *lhs, Value rhs)
{
...
new_clast_reduction (enum clast_red_type t, int n)
{
  int i;
  struct clast_reduction *r;
  r =
    malloc (sizeof (struct clast_reduction) +
	    (n - 1) * sizeof (struct clast_expr *));
...
static void
free_clast_root (struct clast_stmt *s)
{
...
static void
free_clast_assignment (struct clast_stmt *s)
{
...
static void
free_clast_user_stmt (struct clast_stmt *s)
{
...
new_clast_user_stmt (CloogStatement * stmt, struct clast_stmt *subs)
{
...
static void
free_clast_block (struct clast_stmt *s)
{
...
static void
free_clast_for (struct clast_stmt *s)
{
...
struct clast_for *
new_clast_for (const char *it, struct clast_expr *LB,
...
static void
free_clast_guard (struct clast_stmt *s)
{
  int i;
...
void
free_clast_term (struct clast_term *t)
{
  value_clear (t->val);
  free (t);
}
...
void
free_clast_binary (struct clast_binary *b)
{
  value_clear (b->RHS);
...
void
free_clast_reduction (struct clast_reduction *r)
{
  int i;
  for (i = 0; i < r->n; ++i)
...
void
free_clast_expr (struct clast_expr *e)
{
  if (!e)
    return;
  switch (e->type)
...
void
free_clast_stmt (struct clast_stmt *s)
{
  assert (s->op);
  assert (s->op->free);
  s->op->free (s);
...
void
cloog_clast_free (struct clast_stmt *s)
{
...
static int
clast_term_equal (struct clast_term *t1, struct clast_term *t2)
{
  if (t1->var != t2->var)
    return 0;
...
static int
clast_binary_equal (struct clast_binary *b1, struct clast_binary *b2)
{
  if (b1->type != b2->type)
    return 0;
...
static int
clast_reduction_equal (struct clast_reduction *r1, struct clast_reduction *r2)
{
  int i;
...
int
clast_expr_equal (struct clast_expr *e1, struct clast_expr *e2)
{
  if (!e1 && !e2)
    return 1;
  if (!e1 || !e2)
 ...
 */
static int
clast_equal_type (CloogMatrix * equal, int level, int line)
{
  int i, one = 0;
  ...
  for (i = 1; i <= equal->NbColumns - 2; i++)
    if (value_notzero_p (cloog_matrix_element (equal, line, i))
	&& (i != level))
      {
  ...
  if (one != 0)
    {
      if (value_notzero_p
	  (cloog_matrix_element (equal, line, equal->NbColumns - 1)))
	return EQTYPE_EXAFFINE;
    }
  else
 ...
 */
static int
clast_equal_allow (CloogMatrix * equal, int level, int line,
		   CloogInfos * infos)
{
 ...
 */
static int
clast_equal_add (CloogMatrix * equal, CloogMatrix * matrix, int level,
		 int line, CloogInfos * infos)
{
  ...
  if (line == ONE_TIME_LOOP)
	...
	if ((value_notzero_p (cloog_matrix_element (matrix, i, 0))) &&
	    (value_notzero_p (cloog_matrix_element (matrix, i, level))))
	  {
		...
		value_notzero_p (cloog_matrix_element
				 (matrix, i, matrix->NbColumns - 1)))
	      {
		...
		value_assign (denominator,
			      cloog_matrix_element (matrix, i, level));
		value_absolute (denominator, denominator);
		value_assign (numerator,
			      cloog_matrix_element (matrix, i,
						    matrix->NbColumns - 1));
		...
		if (value_pos_p (numerator) || value_zero_p (modulo))
		  cloog_matrix_element_assign (matrix, i,
					       matrix->NbColumns - 1,
					       division);
		else
		  {
		    if (value_pos_p (cloog_matrix_element (matrix, i, level)))
		      cloog_matrix_element_increment (matrix, i,
						      matrix->NbColumns - 1,
						      division);
		    else
		      cloog_matrix_element_decrement (matrix, i,
						      matrix->NbColumns - 1,
						      division);
		  }
   ...
   */
  cloog_matrix_element_set_si (equal, level - 1, 0,
			       clast_equal_type (matrix, level, line));
  /* - the other elements corresponding to the equality itself
   *   (the iterators up to level, then the parameters and the scalar).
   */
  for (i = 1; i <= level; i++)
    cloog_matrix_element_assign (equal, level - 1, i,
				 cloog_matrix_element (matrix, line, i));
  for (i = 0; i < cloog_names_nb_parameters (infos->names) + 1; i++)
    cloog_matrix_element_assign (equal, level - 1, equal->NbColumns - i - 1,
				 cloog_matrix_element (matrix, line,
						       matrix->NbColumns - i -
						       1));

  cloog_matrix_equality_update (equal, level,
				cloog_names_nb_parameters (infos->names));
 ...
 */
static void
clast_equal_del (CloogMatrix * equal, int level)
{
  int i;
 ...
 */
static struct clast_expr *
clast_bound_from_constraint (CloogMatrix * matrix,
			     int line_num, int level, CloogNames * names)
{
  ...
  if (value_notzero_p (cloog_matrix_element (matrix, line_num, level)))
    {
      ...
      sign =
	value_pos_p (cloog_matrix_element (matrix, line_num, level)) ? -1 : 1;

      for (i = 1, nb_elts = 0; i <= matrix->NbColumns - 1; ++i)
	if (i != level
	    && value_notzero_p (cloog_matrix_element (matrix, line_num, i)))
	  nb_elts++;
	...
	if ((i != level)
	    && value_notzero_p (cloog_matrix_element (matrix, line_num, i)))
	  {
	    if (i <= cloog_names_nb_scattering (names))
	      ...
	      name =
		cloog_names_iterator_elt (names,
					  i -
	...
	if ((i != level)
      ...
      if (sign == -1)
	{
	  value_oppose (numerator,
			cloog_matrix_element (matrix, line_num,
					      matrix->NbColumns - 1));
	  value_assign (denominator,
			cloog_matrix_element (matrix, line_num, level));
	}
      else
	{
	  value_assign (numerator,
      ...
      if (nb_elts)
	{
	  ...
	  if (value_notone_p (cloog_matrix_element (matrix, line_num, level))
	      &&
	      value_notmone_p (cloog_matrix_element
			       (matrix, line_num, level)))
	    {
	      if (value_one_p (cloog_matrix_element (matrix, line_num, 0)))
		{
		  if (value_pos_p
		      (cloog_matrix_element (matrix, line_num, level)))
		    e =
		      &new_clast_binary (clast_bin_cdiv, &r->expr,
					 denominator)->expr;
		  else
		    e =
		      &new_clast_binary (clast_bin_fdiv, &r->expr,
					 denominator)->expr;
		}
	      else
		e =
		  &new_clast_binary (clast_bin_div, &r->expr,
				     denominator)->expr;
	    }
	  else
	    e = &r->expr;
  ...
  for (i = cloog_names_nb_scattering (infos->names); i < equal->NbRows; i++)
    {
	  ...
	  && clast_equal_allow (equal, i + 1, i, infos))
	{
	  ...
	  *next =
	    &new_clast_assignment (cloog_names_iterators (infos->names)
				   [iterator], e)->stmt;
	  next = &(*next)->next;
	}
    }
 ...
 */
static struct clast_stmt *
clast_equal_cpp (int level, CloogInfos * infos)
{
  int i;
  Value type;
  struct clast_expr *e;
  ...
  for (i = cloog_names_nb_scattering (infos->names); i < level - 1; i++)
      ...
      else
	{
	  ...
	  e = &new_clast_term
	    (type,
	     cloog_names_iterator_elt (infos->names,
				       i -
 ...
 */
static struct clast_expr *
clast_minmax (CloogMatrix * matrix,
	      int level, int max, int guard, CloogInfos * infos)
{
  int i, n;
  struct clast_reduction *r;
  ...
  for (i = 0, n = 0; i < matrix->NbRows; i++)
	...
	(value_notzero_p (cloog_matrix_element (matrix, i, 0))))
      r->elts[n++] =
	clast_bound_from_constraint (matrix, i, level, infos->names);
 ...
 */
static void
insert_guard (CloogMatrix * matrix, int level,
	      struct clast_stmt ***next, CloogInfos * infos)
{
  ...
  for (i = 1; i <= copy->NbColumns - 2; i++)
    for (j = 0; j < copy->NbRows; j++)
	  ...
	  (value_zero_p (cloog_matrix_element (copy, j, level))
	   || (nb_iter < level)))
	{
	  struct clast_term *t;
	  if (i <= nb_iter)
	  ...
	  if (value_zero_p (cloog_matrix_element (copy, j, 0)))
	    {
	      ...
	      g->eq[nb_and].sign = 0;
	      g->eq[nb_and].RHS =
		clast_bound_from_constraint (copy, j, i, infos->names);
	      ...
	      else
		{
	      ...
	      g->eq[nb_and].RHS =
		clast_minmax (copy, i, minmax, guarded, infos);
	    }
	  nb_and++;
	  ...
	  if (minmax != -1)
	    for (l = j + 1; l < copy->NbRows; l++)
	      if (((minmax == 1)
		   && value_pos_p (cloog_matrix_element (copy, l, i)))
		  || ((minmax == 0)
		      && value_neg_p (cloog_matrix_element (copy, l, i))))
		for (k = i; k <= copy->NbColumns - 2; k++)
...
static void
Euclid (Value a, Value b, Value * x, Value * y, Value * g)
{
  Value c, d, e, f, tmp;
 ...
 */
static void
insert_modulo_guard (CloogMatrix * matrix, int num, int level,
		     struct clast_stmt ***next, CloogInfos * infos)
{
  ...
  if (value_one_p (cloog_matrix_element (matrix, num, level))
      || value_mone_p (cloog_matrix_element (matrix, num, level)))
    return;
  ...
  for (i = nb_iter; i >= 1; --i)
	...
	{
	  if (value_cmp_si
	      (cloog_matrix_element (infos->equal, j, 0),
	       EQTYPE_EXAFFINE) != 0)
	    continue;
	  value_modulus (val, cloog_matrix_element (infos->equal, j, 1 + j),
			 line[level]);
	  if (value_notzero_p (val))
	    continue;
	  value_modulus (val, cloog_matrix_element (infos->equal, j, i),
			 line[level]);
	  if (value_zero_p (val))
	    continue;
	  for (k = j; k > i; --k)
		...
		continue;
	      value_modulus (val, cloog_matrix_element (infos->equal, j, k),
			     line[level]);
	      if (value_notzero_p (val))
		break;
	    }
	  if (k > i)
	    continue;
	  Euclid (cloog_matrix_element (infos->equal, j, i), line[level], &x,
		  &y, &g);
	  ...
	  Vector_Combine (line + 1, infos->equal->p[j] + 1, line + 1, y, val,
			  i);
	  Vector_Combine (line + len - nb_par - 1,
			  infos->equal->p[j] + infos->equal->NbColumns -
			  nb_par - 1, line + len - nb_par - 1, y, val,
			  nb_par + 1);
	  break;
	}
      if (j >= 0)
 ...
 */
static void
insert_equality (CloogMatrix * matrix, int num,
		 int level, struct clast_stmt ***next, CloogInfos * infos)
{
  ...
  if (!clast_equal_add (infos->equal, matrix, level, num, infos))
	...
	ass =
	  new_clast_assignment (cloog_names_scattering_elt
				(infos->names, level - 1), e);
      else
	ass = new_clast_assignment
	  (cloog_names_iterator_elt
	   (infos->names,
	    level - cloog_names_nb_scattering (infos->names) - 1), e);
 ...
 */
static void
insert_for (CloogMatrix * matrix, int level,
	    struct clast_stmt ***next, CloogInfos * infos)
{
  char *iterator;
  ...
  if (!clast_expr_equal (e1, e2) || !infos->options->otl || (!e1 && !e2))
    {
      struct clast_for *f =
	new_clast_for (iterator, e1, e2, infos->stride[level - 1]);
      **next = &f->stmt;
      *next = &f->body;
    }
  else
    if (!clast_equal_add (infos->equal, matrix, level, ONE_TIME_LOOP, infos))
    {
      if (infos->options->block)
	{
	  struct clast_block *b = new_clast_block ();
 ...
 */
static void
insert_scalar (CloogLoop * loop, int level, int *scalar,
	       struct clast_stmt ***next, CloogInfos * infos)
{
  ...
  if ((!infos->options->csp) &&
	     ...
	     (infos->scaldims[level + (*scalar) - 1]))
	{
	  ...
	  t =
	    new_clast_term (cloog_block_scaldims_elt
			    (cloog_loop_block (loop), *scalar), NULL);
	  **next =
	    &new_clast_assignment (cloog_names_scalar_elt
				   (infos->names, *scalar), &t->expr)->stmt;
	  *next = &(**next)->next;
	  (*scalar)++;
 ...
 */
static void
insert_block (CloogBlock * block, int level,
	      struct clast_stmt ***next, CloogInfos * infos)
{
 ...
 */
static void
insert_loop (CloogLoop * loop, int level, int scalar,
	     struct clast_stmt ***next, CloogInfos * infos)
{
  ...
  matrix = cloog_simplify_domain_matrix_with_equalities
    (cloog_loop_domain (loop), level, infos->equal,
     cloog_names_nb_parameters (infos->names));
  ...
  if ((matrix->NbColumns - 2 - cloog_names_nb_parameters (infos->names) >=
       level))
    {
...
cloog_clast_create (CloogProgram * program, CloogOptions * options)
{
  ...
  struct clast_stmt *root =
    &new_clast_root (cloog_program_names (program))->stmt;
  ...
  infos->equal =
    cloog_matrix_alloc (nb_levels,
			nb_levels +
			cloog_names_nb_parameters (cloog_program_names
						   (program)) + 1);
